...
# Helper: override the large font sizes in plot_rcp1arm*() for vignette display.
# The plot functions use size = 28 for publication slides; here we reduce to
# readable sizes for HTML output.
vignette_theme <- function() {
  ggplot2::theme(
...
Let $\theta$ denote the treatment effect parameter for a given endpoint (e.g., mean difference, response rate, log hazard ratio). Method 1 requires that Region 1 retains at least a fraction $\pi$ of the overall treatment effect:
...
with $f_1 = N_1/N$ being the regional allocation fraction and $\hat{\theta}_{-1}$ the pooled estimate for regions $2, \ldots, J$ combined. Under the assumption of homogeneous treatment effects across regions, $D$ follows a normal distribution with mean $(1-\pi)\delta$ and a variance that depends on the endpoint type, yielding a closed-form expression for $\text{RCP}_1$.
...
| True mean | $\mu = 0.5$ |
| Historical control mean | $\mu_0 = 0.1$ (treatment effect $\delta = 0.4$) |
...
- **Method 1** (blue) is relatively insensitive to $f_1$ over a wide range, because the two-block decomposition evaluates Region 1 against the pooled remainder.
- **Method 2** (yellow) decreases monotonically as $f_1$ decreases (Region 1 becomes smaller), since $\Pr(\hat{\theta}_1 > \theta_0)$ diminishes with smaller $N_1$.
