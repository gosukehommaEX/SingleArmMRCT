...
**Closed-form solution when $t_{\text{eval}} \leq t_f$:**

When $G_a(u) = 1$ throughout $[0, t_{\text{eval}}]$:
...
For $\lambda_d = 0$ this reduces to the binomial variance $S(t)(1 - S(t))$.

**Numerical integration when $t_{\text{eval}} > t_f$:** the integral is split at $t_f$ and evaluated via `stats::integrate()`.
...
**Closed-form solution when $\tau^* \leq t_f$:**

Expanding the squared term and integrating with $A(r) = (e^{r\tau^*} - 1)/r$ (and $A(0) = \tau^*$):
...
**Numerical integration when $\tau^* > t_f$:** the integral is split at $t_f$ and evaluated via `stats::integrate()`.
