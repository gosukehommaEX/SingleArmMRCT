...
where $b(y;\,n,\,p) = \binom{n}{y}p^y(1-p)^{n-y}$ is the binomial probability mass function.

**Method 2 (Simultaneous Positivity):**

The condition $\hat{p}_j > p_0$ is equivalent to $Y_j \geq y_{j,\min}$ where $y_{j,\min} = \lfloor N_j p_0 \rfloor + 1$. Therefore:

$$
\text{RCP}_2 = \prod_{j=1}^{J} \Pr(Y_j \geq y_{j,\min})
= \prod_{j=1}^{J} \left[1 - F_{\mathrm{Bin}(N_j,\,p)}(y_{j,\min} - 1)\right]
$$
