...
function of the regional allocation proportion \\f_1\\ for restricted
mean survival time (RMST) endpoints. Formula and simulation results are
shown together for both Method 1 and Method 2. Facet columns correspond
to total sample sizes specified in `N_vec`.
...
- mu0:
  ...
  less than `tau_star`. Default is
  `(1 - exp(-log(2) / 5 * 8)) / (log(2) / 5)`.
