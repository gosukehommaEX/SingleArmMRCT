...
#' of the regional allocation proportion \eqn{f_1} for binary endpoints.
...
#' @param p0 Numeric scalar. Null hypothesis response rate. Must be in \eqn{[0, 1)}.
#'   Default is \code{0.2}.
...
#'
#' @export
plot_rcp1armBinary <- function(p      = 0.5,
                               p0     = 0.2,
                               PI     = 0.5,
                               N_vec  = c(20, 40, 100),
                               J      = 3,
  ...
  plt <- ggplot2::ggplot(df, ggplot2::aes(x = f1, y = RCP,
    ...
    ggplot2::labs(
      title = substitute(
        paste("Binary Endpoint  (",
              italic(p) == a, ", ", italic(p)[0] == b, ", ",
              pi == d, ", ", italic(J) == e, ")"),
        list(a = p, b = p0, d = PI, e = J)
      ),
